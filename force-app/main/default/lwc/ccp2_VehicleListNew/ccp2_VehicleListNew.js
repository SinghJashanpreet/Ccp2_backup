...
export default class Ccp2_VehicleListNew extends LightningElement {
  ...
  @track AllFlag = true;
  @track DownloadName = "日付- カスタマーポータル車両リスト";
  ...
  @track LeaseFlag = false;
  @track VehicleInspExpNotfFlag = false;
  ...
  @track CheckedFirst = false;
  @track CheckedSecond = false;
  @track CheckedThird = false;

  vehicleBrachCount = 0;
  vehicleAccountCount = 0;
  @track ownedVehicleCount = 0;

  @track CarTypesPicklistValues = [
    { label: "すべて", value: "すべて", selected: false },
    { label: "大型トラック", value: "大型トラック", selected: false },
    { label: "中型トラック", value: "中型トラック", selected: false },
    { label: "小型トラック", value: "小型トラック", selected: false }
  ];
  ...
  get isRegDiffAbove() {
    return this.jsonParameterForVehicleClassWithoutString.RegDiff[1] === 11;
  }

  get milageMin() {
    let milageMini = this.jsonParameterForVehicleClassWithoutString.Milage[0];
    milageMini = milageMini.toLocaleString("en-US");
    return milageMini;
  }

  get milageMax() {
    let returnVal =
      this.jsonParameterForVehicleClassWithoutString.Milage[1] === 1000001
        ? 1000000
        : this.jsonParameterForVehicleClassWithoutString.Milage[1];
    returnVal = returnVal.toLocaleString("en-US");
    return returnVal;
  }
  get expMin() {
  ...
  get isShowVehicleFilterPills() {
    return (
      ...
      this.jsonParameterForVehicleClassWithoutString.finalCarTypeList ||
      ...
      this.jsonParameterForVehicleClassWithoutString.VehicleInspExpNotf ||
      this.jsonParameterForVehicleClassWithoutString.Lease ||
  ...
  renderedCallback() {
    ...
    if (!this.outsideClickHandlerAdded3) {
      document.addEventListener(
        "click",
        this.handleOutsideClick3dev.bind(this)
      );
  ...
  Vehicledata(result) {
    ...
    if (data) {
      // console.log("Jsonfile", this.jsonParameterForVehicleClass, 'limitRange', this.currentPagination, 'pageNo', this.currentPage, 'Sorting', this.finalsortingValue, 'uiSearch', this.filterSuggestionsToSend);

      // console.log("full vehicle wire data without objectify:- ", data);
      // console.log("full vehicle wire data with objectify:- ", JSON.stringify(data));
      ...
      this.ownedVehicleCount = data[0]?.VehicleCountBranch || 0;
      this.vehicleAccountCount = data[0]?.totalVehicleActiveCount || 0;
      ...
      this.vehicleData = data.map((item) => {
        ...
        let expDate = vehicle?.Vehicle_Expiration_Date__c
          ...
          : "-";
        let FirstRegDate = vehicle?.First_Registration_Date__c
          ? this.formatJapaneseDate2(vehicle.First_Registration_Date__c)
          : "-";

        let SubBrandName = vehicle?.Sub_Brand_Name__c || "-";

        let Mileage = this.formatMileage(vehicle.Mileage__c);
        ...
        return {
          ...
          ActiveLease,
          onlyregNumellp,
          showAlertrecallNdShaken,
          showAlerttruckorlease,
          imageSrc,
          showRecallM,
      ...
      ErrorLog({
        lwcName: "ccp2_vehicleListNew",
        errorLog: error2,
        MethodName: "Main List Method",
        ViewName: "Vehicle List",
  ...
  totalCountFun(result) {
    ...
    if (data) {
      ...
      ErrorLog({
        lwcName: "ccp2_vehicleListNew",
        errorLog: error,
        methodName: "total count class",
        ViewName: "Vehicle List",
        InterfaceName: "CCP User Interface",
        EventName: "Count Pagination",
        ModuleName: "VehicleManagement"
      })
        .then(() => {
          console.log("Error logged successfully in Salesforce");
  ...
  branchApiFun(result) {
    ...
    if (data) {
      ...
      if (this.finalFilterJson.isAllBrnaches === "true") {
        console.log("in all branches tick!!!");
        let data2 = data.map((elm) => {
          return { ...elm, selected: true };
        });

        this.branchDataToSendBack = data2.map((b) => b.branchId);
        ...
        this.branchOptions = [
          { branchId: "すべて", branchName: "すべて", selected: true },
          ...data2
        ];
      }
    } else if (err) {
      ...
      ErrorLog({
        lwcName: "ccp2_vehicleListNew",
        errorLog: error,
        methodName: "Branch Options Picklists",
        ViewName: "Vehicle List",
        InterfaceName: "CCP User Interface",
        EventName: "Data fetch",
        ModuleName: "VehicleManagement"
  ...
  searchDataFun({ data, err }) {
    if (data) {
      ...
      ErrorLog({
        lwcName: "ccp2_vehicleListNew",
        errorLog: error,
        methodName: "Search function method class",
        ViewName: "Vehicle List",
        InterfaceName: "CCP User Interface",
        EventName: "Data fetch",
        ModuleName: "VehicleManagement"
    ...
    listBySearchVehicle({ lastDigits: regOrDoorNumberSearched })
      ...
      .catch((err) => {
        ...
        ErrorLog({
          lwcName: "ccp2_vehicleListNew",
          errorLog: error,
          methodName: "CCP2_VehicleDetailController.getVehicleOnSearch",
          ViewName: "Vehicle List",
          InterfaceName: "CCP User Interface",
          EventName: "Data fetch",
          ModuleName: "VehicleManagement"
  ...
  connectedCallback() {
    ...
    const urlParamsVehicleId = new URLSearchParams(window.location.search).get(
      "vehicleId"
    );
    // const urlParamsPage = new URLSearchParams(window.location.search).get(
    //   "page"
    // );
    const urlParamsFilter = new URLSearchParams(window.location.search).get(
      "filter"
    );
    if (urlParamsVehicleId) {
      ...
      if (urlParamsFilter === "lease") {
      ...
      } else if (urlParamsFilter === "reg_difference") {
        this.slider3check = true;
        this.sizelist = true;
        ...
        // this.finalFilterJson = { ...this.finalFilterJson, Lease: "true" };
      } else if (urlParamsFilter === "vehicle_expiry_notification") {
        this.VehicleInspExpNotfFlag = true;
        this.finalFilterJson = {
          ...this.finalFilterJson,
          VehicleInspExpNotf: "true"
        };
      }
      this.handleFilterSave();
    }
    // else if (urlParamsPage) {
    //   console.log("urlParamsPage", urlParamsPage);
    //   this.currentPage = Number(urlParamsPage);
    //   this.updatePageButtons();
    // }
    ...
    document.addEventListener("click", this.handleOutsideClick3dev);
  ...
  handlecardClick(event) {
    ...
    if (this.deletecheckboxoverfav) {
      ...
      let url = `/s/vehiclemanagement?vehicleId=${this.vehicleId}&instance=details`;
      console.log("Clicked Vehicle id:", url);
  ...
  updatePageButtons() {
    ...
    buttons.forEach((button) => {
      ...
      console.log("Page Number Avail:- : ", pageNum, this.currentPage);
  ...
  updateVisiblePages() {
    ...
    if (this.totalPageCount === 5) {
      startPage = 1;
      endPage = Math.min(5, this.totalPageCount);
    } else {
  ...
  showDownloadModal() {
    ...
    this.DownloadName = `${this.currentDate} - カスタマーポータル車両リスト`;
  }
  closeDownload() {
      ...
      getFilterVehicleForDownload({
        ...
        .catch((error) => {
          ...
          ErrorLog({
            lwcName: "ccp2_vehicleListNew",
            errorLog: error2,
            methodName: "download filter vehicles method",
            ViewName: "Vehicle List",
            InterfaceName: "CCP User Interface",
            EventName: "Data download",
            ModuleName: "VehicleManagement"
          })
            .then(() => {
              console.log("Error logged successfully in Salesforce");
  ...
  downloadCSVAll() {
    ...
    const csvRows = this.allVehiclesData.map((record) => {
      ...
      return [
        ...
        vehicle.Body_Shape_Text__c || "",
        vehicle.vehicleWeight__c || "",
        vehicle.First_Registration_Date__c || "",
        vehicle.Vehicle_Expiration_Date__c || "",
        vehicle.Mileage__c || "0",
        vehicle.Fuel_Type_Text__c || "",
        vehicle.Private_Business_Text__c || "",
        vehicle.Use_Text__c || "",
        vehicle.fullModel__c || "",
        //vehicle.Door_Number__c || "",
        branches,
        vehicle.Status__c === "CurrentlyOwned" ? " " : "削除済み" || "",
        vehicle.Reg_Vehicle_Size__c || "",
        vehicle.VehicleGrossWeight__c || "",
        vehicle.Payload__c || ""
      ];
    });
  ...
  openFilterModal() {
    ...
    window.scrollTo(0, 0);
    console.log("called-overflow", document.body.style.overflow);
  }
  ...
  handleCarTypeSelect(event) {
    ...
    let lengthOfList = 0;
    if (value === "すべて") {
      this.finalCarTypeList = [];
      this.CarTypesPicklistValues = this.CarTypesPicklistValues.map((elm) => {
        if (elm.label !== "すべて") {
          this.finalCarTypeList.push(elm.label);
        }
        return { ...elm, selected: isChecked };
      });
      if (!isChecked) {
      ...
      lengthOfList = this.finalCarTypeList.length;
    } else {
      this.CarTypesPicklistValues = this.CarTypesPicklistValues.map((elm) => {
        if (elm.label === value) {
          return { ...elm, selected: isChecked };
      ...
      if (isChecked) {
        if (!this.finalCarTypeList.includes(value)) {
          this.finalCarTypeList = [...this.finalCarTypeList, value];
        }
      } else {
        this.finalCarTypeList = this.finalCarTypeList.filter(
          (item) => item !== value
        );
      }
      let isAllSelected = this.CarTypesPicklistValues.filter(
        (elm) => elm.label !== "すべて"
      ).every((item) => item.selected);
      this.CarTypesPicklistValues = this.CarTypesPicklistValues.map((elm) => {
        if (elm.label === "すべて") {
      ...
      lengthOfList = this.CarTypesPicklistValues.filter(
        (elm) => elm.selected === true && elm.label !== "すべて"
      ).length;
    }

    this.CarTypeValue = lengthOfList === 0 ? "" : lengthOfList + "件選択中";
    if (this.CarTypeValue.length) {
      this.finalFilterJson = {
        ...this.finalFilterJson,
        finalCarTypeList: this.finalCarTypeList
      };
    } else {
      // eslint-disable-next-line no-unused-vars
      const { finalCarTypeList, ...rest } = this.finalFilterJson;
      this.finalFilterJson = rest;
    }
    console.log(
      "this.finalCarTypeList",
      JSON.stringify(this.finalCarTypeList),
      "this.finalCarTypeList",
      JSON.stringify(this.finalVehicleTypeList),
      "this.finalVehicleNameList",
      JSON.stringify(this.finalVehicleNameList)
  ...
  handleCheckboxChangeForPills(event) {
    ...
    if (value === "Delete") {
      ...
      this.LeaseFlag = event.target.checked;
      if (event.target.checked) {
        this.finalFilterJson = { ...this.finalFilterJson, [value]: "true" };
      } else {
        const { Lease, ...rest } = this.finalFilterJson;
        this.finalFilterJson = rest;
      }
    } else if (value === "VehicleInspExpNotf") {
      this.VehicleInspExpNotfFlag = event.target.checked;
      if (event.target.checked) {
        this.finalFilterJson = { ...this.finalFilterJson, [value]: "true" };
      } else {
        const { VehicleInspExpNotf, ...rest } = this.finalFilterJson;
        this.finalFilterJson = rest;
      }
    } else {
  ...
  removePill(event) {
    ...
    if (key === "Milage" || key === "RegDiff" || key === "ExpDiff") {
      ...
      this.LeaseFlag = false;
      // eslint-disable-next-line no-unused-vars
      const { Lease: Main, ...restClass } = this.finalFilterJsonForClass;
      this.finalFilterJsonForClass = restClass;
      // eslint-disable-next-line no-unused-vars
      const { Lease: Main2, ...restMain } = this.finalFilterJson;
      this.finalFilterJson = restMain;
    } else if (key === "VehicleInspExpNotf") {
      this.VehicleInspExpNotfFlag = false;
      // eslint-disable-next-line no-unused-vars
      const { VehicleInspExpNotf: Main, ...restClass } =
        this.finalFilterJsonForClass;
      this.finalFilterJsonForClass = restClass;
      // eslint-disable-next-line no-unused-vars
      const { VehicleInspExpNotf: Main2, ...restMain } = this.finalFilterJson;
      this.finalFilterJson = restMain;
    } else if (key === "LargeTrucks") {
      this.CheckedFirst = false;
      // eslint-disable-next-line no-unused-vars
      const { LargeTruck: TKL, ...restClass } = this.finalFilterJsonForClass;
      this.finalFilterJsonForClass = restClass;
      // eslint-disable-next-line no-unused-vars
      const { LargeTruck: TKL2, ...restMain } = this.finalFilterJson;
      this.finalFilterJson = restMain;
    } else if (key === "MediumTrucks") {
      this.CheckedSecond = false;
      // eslint-disable-next-line no-unused-vars
      const { MediumTruck: TKM, ...restClass } = this.finalFilterJsonForClass;
      this.finalFilterJsonForClass = restClass;
      // eslint-disable-next-line no-unused-vars
      const { MediumTruck: TKM2, ...restMain } = this.finalFilterJson;
      this.finalFilterJson = restMain;
    } else if (key === "SmallTrucks") {
      this.CheckedThird = false;
      // eslint-disable-next-line no-unused-vars
      const { SmallTruck: TKS, ...restClass } = this.finalFilterJsonForClass;
      this.finalFilterJsonForClass = restClass;
      // eslint-disable-next-line no-unused-vars
      const { SmallTruck: TKS2, ...restMain } = this.finalFilterJson;
      this.finalFilterJson = restMain;
    } else {
      ...
      switch (key) {
        ...
        case "finalCarTypeList": {
          this.CarTypesPicklistValues = this.CarTypesPicklistValues.map(
            (elm) => {
              if (elm.label === value) {
                return { ...elm, selected: false };
              } else if (elm.label === "すべて") {
          ...
          let lengthOfList2 = this.CarTypesPicklistValues.filter(
            (elm) => elm.selected === true && elm.label !== "すべて"
          ).length;
  ...
  updateFloatingValues(id) {
    ...
    if (outputOne && outputTwo) {
      const multiplier = this.sliderConfigs[id].multiplier;
      let leftValNum11 = Math.ceil(
        this.rangeValues[id].lower * multiplier
      ).toFixed(0);
      let leftVal11 =
        this.formatCurrency(
          leftValNum11 == 0 && id != "Slider1" ? 1 : leftValNum11
        ) +
        " " +
        (id == "Slider3" && leftValNum11 == 0
          ? "年以内"
          : id == "Slider2" && leftValNum11 == 0
            ? "か月以内"
            : suffix);
      outputOne.textContent = leftVal11;
      let rightValNum11 = Math.ceil(
        this.rangeValues[id].upper * multiplier
      ).toFixed(0);
      let rightVal11 =
        this.formatCurrency(rightValNum11) +
        " " +
        (id == "Slider3" && rightValNum11 == 10
          ? "年以内"
          : id == "Slider2" && rightValNum11 == 12
            ? "か月以内"
            : suffix);
      outputTwo.textContent = rightVal11;
  ...
  wiredVehicleTypePicklistValues({ error, data }) {
    if (data) {
      ...
      ErrorLog({
        lwcName: "ccp2_vehicleListNew",
        errorLog: error2,
        methodName: "VEHICLE TYPE Picklist",
        ViewName: "Vehicle List",
        InterfaceName: "CCP User Interface",
        EventName: "Data fetch",
        ModuleName: "VehicleManagement"
  ...
  handleResetofFilter() {
    ...
    delete this.finalFilterJson.finalCarTypeList;
    delete this.finalFilterJson.Delete;
    delete this.finalFilterJson.Recall;
    delete this.finalFilterJson.Lease;
    delete this.finalFilterJson.TruckKonnect;
    delete this.finalFilterJson.VehicleInspExpNotf;
  ...
  handleAllRangeSelected(event) {
    ...
    if (checkboxId === "checkboxslider1") {
      ...
      if (isCheckedd) {
        ...
        this.CheckedSecond = false;
        this.CheckedThird = false;
        const { RegDiff, LargeTruck, MediumTruck, SmallTruck, ...rest } =
          this.finalFilterJson;
        this.finalFilterJson = rest;
        console.log("filterjson uncheck", JSON.stringify(this.finalFilterJson));
      }
    }
  ...
  //     this.CheckedFirst = event.target.checked;
  //     if (isCheckedd) {
  //       this.finalFilterJson = {
  //         ...this.finalFilterJson,
  //         LargeTruck : "LargeTruck"
  ...
  //     } else {
  //       // eslint-disable-next-line no-unused-vars
  //       const { LargeTruck, ...rest } = this.finalFilterJson;
  //       this.finalFilterJson = rest;
  //     }
  //   } else if (checkboxId === "check2") {
  ...
  //     } else {
  //       // eslint-disable-next-line no-unused-vars
  //       const { MediumTruck, ...rest } = this.finalFilterJson;
  //       this.finalFilterJson = rest;
  //     }
  //   } else if (checkboxId === "check3") {
  ...
  //       const { SmallTruck, ...rest } = this.finalFilterJson;
  //       this.finalFilterJson = rest;
  //     }
  //   }

  //   // Log to verify
  //   console.log("filterjson:2",JSON.stringify(this.finalFilterJson));
  //   console.log("finalfinlterjson:2", JSON.stringify(this.finalFilterJsonForClass));
  // }
      ...
      .then((result) => {
        ...
        if (Array.isArray(result) && result.length > 0) {
          ...
          ErrorLog({
            lwcName: "ccp2_vehicleListNew",
            errorLog: error,
            methodName: "handledeleteopenmodal",
            ViewName: "Vehicle List",
            InterfaceName: "CCP User Interface",
            EventName: "Data fetch",
            ModuleName: "VehicleManagement"
    ...
    deletesavevehicle({
      ...
      .catch((err) => {
        ...
        ErrorLog({
          lwcName: "ccp2_vehicleListNew",
          errorLog: error,
          methodName: "FINAL DELETE VEHICLE"
        })
          .then(() => {
            console.log("Error logged successfully in Salesforce");
          })
  ...
  handleBranchSelect(event) {
    const { isAllBrnaches: first, ...rest } = this.finalFilterJson;
    this.finalFilterJson = rest;

    const { isAllBrnaches: second, ...rest2 } = this.finalFilterJsonForClass;
    this.finalFilterJsonForClass = rest2;

    this.currentPage = 1;
    const branchId = event.target.name;
    const isChecked = event.target.checked;
    console.log("at start this.finalFilterJson", this.finalFilterJson);
  ...
  handleFilterSave() {
    ...
    if (
      (urlParamsFilter === "lease" &&
        !this.jsonParameterForVehicleClassWithoutString.Lease) ||
      (urlParamsFilter === "reg_difference" &&
        !this.jsonParameterForVehicleClassWithoutString.RegDiff) ||
      (urlParamsFilter === "vehicle_expiry_notification" &&
        !this.jsonParameterForVehicleClassWithoutString.VehicleInspExpNotf)
    ) {
      const newUrl = "/s/vehiclemanagement";
  ...
  handleSortChange(event) {
    ...
    this.nameofsort = event.target.name;
    console.log("kaun", this.nameofsort);
  ...
  FinalSorttoSend() {
    ...
    this.finalsortname = this.nameofsort;
    this.updateButtonVisibility();
    this.showSortingModal = false;
  }
  ...
  toggleCarType(event) {
    event.stopPropagation();
    this.showCarTypeDropdown = !this.showCarTypeDropdown;
    this.showVehicleNameDropdown = false;
    this.showVehicleTypeDropdown = false;
  }
