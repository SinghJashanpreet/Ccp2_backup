...
export default class Ccp_RegisterAdminUser extends LightningElement {
  ...
  handleInputChange(event) {
    ...
    if (event.target.type === "checkbox") {
      ...
      if (
        (fieldName === "vehicleService" ||
          fieldName === "invoiceService" ||
          fieldName === "leaseLoanService") &&
    ...
    this.corporateRegistrationButton =
      this.siebelAccountCode && this.postalCodePrefix && this.postalCodeSuffix;
    // ユーザの登録で必須項目が入力された場合、ボタンを有効化
    this.userRegistrationButton =
      this.lastNameKana &&
      this.firstNameKana &&
  ...
  corporateNextClick() {
    ...
    if (!this.postalCodePrefix) {
      ...
      !onlyNumber.test(this.postalCodePrefix) ||
      this.postalCodePrefix.length !== 3
    ) {
      this.postalCodeFrontError =
        "入力内容に誤りがあります。内容をご確認ください。";
    }
    ...
    if (!this.postalCodeSuffix) {
      ...
      !onlyNumber.test(this.postalCodeSuffix) ||
      this.postalCodeSuffix.length !== 4
    ) {
      this.postalCodeBackError =
        "入力内容に誤りがあります。内容をご確認ください。";
    }
  ...
  contactNextClick() {
    ...
    checkSiebelAccountCode({
      siebelAccountCode: this.corporateData.siebelAccountCode
    })
      .then((result) => {
  ...
  async checkLeaseLoanStatus() {
    try {
      const result = await hasActiveLeaseLoanAccounts({
        headOfficeId: this.corporateData.headOfficeId
      });
  ...
  async onConfirmServiceNextButtonClick() {
    ...
    console.log(
      "Type of this.invoiceService.checked:",
      typeof this.invoiceService
    );
    console.log(
      "Type of this.invoiceService.checked:",
      typeof this.leaseLoanService
    );
    getTerritoryEmployeeStatus({ accountId: this.corporateData.accountId })
      .then((result) => {
        ...
        if (!this.territoryEmployeeFlag && this.invoiceService) {
          ...
          sendNoTerritoryManagerNotificationEmail({
            accountId: this.corporateData.accountId,
            contactInputData: contactToUpdate
          })
            .then(() => {
      ...
      .catch((error) => {
        console.error(
          "getTerritoryEmployeeStatusの実行中にエラーが発生:",
          error
        );
      });
    ...
    await updateContact({
      ...
      .then(async () => {
        ...
        //権限セット付与
        await setPermissionSetForManagerUser({
          vehicleService: this.vehicleService,
          invoiceService: this.invoiceService,
          leaseLoanService: this.leaseLoanService
        });

        //e-invoice申込通知メールをSC担当者に送信
        sendCCPSCContactEmail({
          accountId: this.corporateData.accountId,
        ...
        if (
          ...
          await updateAccessControlAsync({
            contactId: contactToUpdate.Id,
            vehicleService: this.vehicleService,
            invoiceService: this.invoiceService,
            leaseLoanService: this.leaseLoanService
  ...
  onServiceRegistrationNextButtonClick() {
    ...
    if (!this.invoiceService && !this.leaseLoanService) {
      ...
      if (!this.regNumClassificationNumber) {
        ...
        this.regNumClassificationNumber.length > VEHICLE_NAME2_LENGTH
      ) {
        this.regNumClassificationNumberError =
          "入力内容に誤りがあります。内容をご確認ください。";
      ...
      if (
        ...
        // 4つの名前を結合
        this.leaseLoanRegisteredVehicleName =
          this.regNumPlaceName +
          this.regNumClassificationNumber +
          this.regNumHiragana +
          this.regNumSerialNumber;
