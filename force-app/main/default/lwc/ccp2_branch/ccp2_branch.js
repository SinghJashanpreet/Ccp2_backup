...
export default class Ccp2Branch extends NavigationMixin(LightningElement) {
    ...
    loadLabels() {
        fetch(`${labelsBranch}/labelsBranch.json`)
            ...
            .catch((error) => {
                ErrorLog({ lwcName: "ccp2_branch", errorLog: error, methodName: "loadLabels" })
                    .then(() => {
                        console.log("Error logged successfully in Salesforce");
                    })
                    .catch((loggingErr) => {
    ...
    fetchBranchData() {
        BranchVehicleCount()
            ...
            .catch(error => {
                ErrorLog({ lwcName: "ccp2_branch", errorLog: error, methodName: "fetchBranchData" })
                    .then(() => console.log("Error logged successfully in Salesforce"))
                    .catch(loggingErr => console.error("Failed to log error in Salesforce:", loggingErr));
                console.error('Error loading branches:', error);
            });
    }
    ...
    updateVisiblePageNumbers() {
        ...
        if (totalPages <= 5) {
            ...
            this.visiblePageNumbers = [totalPages - 3, totalPages - 2, totalPages - 1, totalPages];
            this.showLeftDots = true;
            this.showRightDots = false;
        } else {
    ...
    fetchItems(page) {
        ...
        this.paginatedBranchData = this.branchData.slice(startIdx, endIdx).map(branch => ({
            ...branch,
            formattedSiebelAccountCode: branch.SiebelAccountCode || "未登録",
            formattedBranchNO: branch.BranchNO ? branch.BranchNO.toString().padStart(3, '0') : ""
        }));
    ...
    loadLanguage() {
        Languagei18n() // Assuming getLanguageI18n is the apex method that fetches the language.
            ...
            .catch((error) => {
                ErrorLog({ lwcName: "ccp2_branch", errorLog: error, methodName: "loadLanguage" })
                    .then(() => {
                        console.log("Error logged successfully in Salesforce");
                    })
                    .catch((loggingErr) => {
    ...
    wiredBranchesNo({ error, data }) {
        if (data) {
            ...
            const hasPriorityBranch = this.branchData.some(branch => branch.BranchNO === 1);
            ...
            ErrorLog({ lwcName: "ccp2_branch", errorLog: error, methodName: "WiredBranchesNo" })
                .then(() => {
                    console.log("Error logged successfully in Salesforce");
                })
                .catch((loggingErr) => {
