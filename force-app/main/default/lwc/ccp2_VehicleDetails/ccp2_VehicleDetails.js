...
export default class Ccp2_VehicleDetails extends LightningElement {
    ...
    if (data) {
      ...
      if (data.length !== 0) {
        let obj = {
          ...
          name:
            data?.Vehicle_Name__c === undefined
              ? "-"
              : data?.Vehicle_Name__c,
          type:
            data?.Vehicle_Type__c === undefined
              ? "-"
              : data?.Vehicle_Type__c,
          ...
          firstRegistrationDate:
            data?.First_Registration_Date__c === undefined
              ? "-"
              : this.formatJapaneseDate(data?.First_Registration_Date__c), // Apply Japanese date formatting
          ...
          model:
            data?.fullModel__c === undefined ? "-" : data?.fullModel__c,
          ...
          truckConnect:
            ...
            branchInfo: data?.Branch_Vehicle_Junctions__r?.length === 0 ? [] : data?.Branch_Vehicle_Junctions__r
        };
  ...
  loadbranches(data) {
    ...
    //   .then((data) => {
        ...
        this.vehId = this.vehicleId;
        console.log(
          "Getting Branch from vehicle by Id API2: ",(data),'data.length', data.length
        );
        if (data.length !== 0) {
          console.log(
            "Getting Branch from vehicle by Id API3: ",(data),'data.length', data.length
          );
          this.Branches = data;
          ...
          this.Branches = data.map((branch) => ({
            ...
            BranchName__c:
              branch.BranchName__c.length > 11
                ? `${branch.BranchName__c.slice(0, 7)}...`
  ...
  branchApiFun(result) {
    ...
    if (data) {
      ...
      this.originalbranchoptions = data;
      let lengthOfList = 0 ;
      data.map((a)=>{
        if(a.selected === true){
          lengthOfList++;
      ...
      this.branchOptions = this.branchOptions.map(
        (elm) => {
  ...
  handleImages(result) {
    ...
    if (data) {
      try {
        ...
        // Handle Certificates
        if (Array.isArray(parsedData.Certificates) && parsedData.Certificates.length > 0) {
  ...
  disconnectedCallback() {
    ...
    document.removeEventListener("click", this.handleOutsideClickBranch.bind(this));
  ...
  get ismileagenull() {
    return this.vehicleByIdData.Newmileage === '' ? true : false;
  }
  ...
  closemileageedit(){
    console.log('this.vehicleByIdData.Newmileage',this.vehicleByIdData.Newmileage )
    this.vehicleByIdData.Newmileage = this.vehicleByIdData.mileage.replace(/,/g, '');
    console.log('this.vehicleByIdData.mileage',this.vehicleByIdData.mileage.replace(/,/g, '') )
  ...
  closeBranchedit(){
    ...
    this.branchOptions = this.originalbranchoptions.map((a)=>{
      if(a.selected === true){
        lengthOfList++;
      }
      return a;
    ...
    this.branchOptions.unshift({ branchId: "すべて", branchName: "すべて", selected: isAllSelected });
  ...
  SaveMileagefield(event){
    ...
    const payload = {
      vehicleId: this.vehicleId,
      Mileage__c: this.vehicleByIdData.Newmileage
    };
  ...
  Savebranchfield(){
    //yet to complete
    this.isallbranchesnull = this.branchOptions.every(branch => !branch.selected);
    let branchIdsToSend = this.branchOptions.filter((a)=>{
