...
import getInquiryType from "@salesforce/apex/CCP2_userController.getInquiryType";
...
export default class Ccp2_Enquiry extends LightningElement {
  ...
  //     ErrorLog({
  //       lwcName: "ccp2_Enquiry", errorLog: error, methodName: "inquiryType__c",
  //       ViewName: "Enquiry",
  ...
  // @wire(getInquiryType)
  // fetPicklistValues(result) {
  //   this.picklistVals = result;
  //   const { data, error: wireError } = result; // Rename 'error' to wireError
  //   if (data) {
  //     console.log("inquiry values: ", JSON.parse(JSON.stringify(data)));
  //   } else if (wireError) { // Use wireError instead
  //     console.error("Error fetching user info:", wireError);
  //     let errorDetails = JSON.stringify(wireError); // Renamed for clarity
  //     ErrorLog({
  //       lwcName: "ccp2_Enquiry",
  //       errorLog: errorDetails,
  //       methodName: "Inquirytype",
  //       ViewName: "Enquiry",
  //       InterfaceName: "Salesforce",
  //       EventName: "Data fetch",
  //       ModuleName: "Enquiry"
  ...
  @wire(getInquiryType)
  fetPicklistValues({ data, error }) {
    if (data) {
      console.log("Inquiry vals: ", data);
      this.dummyOptions = data.map((item) => ({
        label: item,
