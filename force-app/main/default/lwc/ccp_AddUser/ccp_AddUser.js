...
export default class Ccp_AddUser extends LightningElement {
    ...
    fetch(`${labelsUser}/labelsUser.json`)
      ...
      .then(data => {
        ...
        i18next.init({
          ...
          console.log("Add User Locale: ", userLocale);
          console.log("Add User Labels: ", this.labels2);
        });
      })
      .catch((err) => {
  ...
  renderedCallback() {
    ...
    ongoingTransactions.userCreateTxn = true;
  ...
  nextClick() {
    ...
    if (
      ...
      this.dispatchEvent(
        new ShowToastEvent({
          ...
          message:
            this.labels2.ccp_up_requiredField7,
          variant: "error"
        })
      )
    ...
    if (!email.value) {
      ...
      checkUserEmail({ email: email.value })
        .then((data) => {
          if (data != null && data != "" && data != undefined) {
            if (data[0] == "false" && data[1] == "false") {
              ...
              this.emailError = true;
              this.emailErrorText =
                this.labels2.ccp_up_emailAlreadyUsed7;
            } else if (data[1] == "true") {
              email.className =
                "form-input _error slds-form-element__control slds-input";
              this.emailError = true;
        ...
        .catch((err) => {
          console.log("checkUserEmail Errors:" + JSON.stringify(err));
          let error = JSON.stringify(err);
        ErrorLog({
          lwcName: "ccp_AddUser",
          errorLog: error,
  ...
  userTypeJudgment() {
    userTypeJudgment()
      ...
      .catch((err) => {
        console.log("userTypeJudgment errors:" + JSON.stringify(err));
        let error = JSON.stringify(err);
        ErrorLog({
          lwcName: "ccp_AddUser",
          errorLog: error,
