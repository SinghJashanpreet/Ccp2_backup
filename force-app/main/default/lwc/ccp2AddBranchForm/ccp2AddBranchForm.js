...
export default class Ccp2AddBranchForm extends LightningElement {
        ...
        for (let i = 0; i < this.users.length; i++) {
            if (this.users[i].value === this.selectedUserId) {
                ...
                this.users = this.users.filter(
                    (memb) => memb.value !== this.selectedUserId
                );
        ...
        AddBranch(params)
            ...
            .catch((error) => {
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        message:
                            "ブランチレコードの挿入中にエラーが発生しました。もう一度試してください" +
                            error.body.message,
                        variant: "error"
                    })
                );
    ...
    handleBranchNameChange(event) {
        ...
        if (regexJapanese.test(input)) {
            ...
            this.errorMessage =
                "Invalid input: Only Japanese characters are allowed, and the length should be up to 24 characters.";
        }
    ...
    handleDeleteUser(event) {
        ...
        const deletedUserFromMoreUsersArray = this.moreusers.find(
            (memb) => memb.Id === userId
        );
        ...
        if (deletedUserFromMoreUsersArray) {
            this.users = [
                ...this.users,
                {
                    label: deletedUserFromMoreUsersArray.Name,
                    value: deletedUserFromMoreUsersArray.Id
                }
            ];
        }
    }
