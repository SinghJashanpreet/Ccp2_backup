...
export default class Ccp2VehicleModal extends LightningElement {
  ...
  @wire(ChassisList) newfile({ data, error }) {
    if (data) {
      ...
      this.optVehicles = data.map((contact) => {
        return {
          label: contact.carPlatformNo__c,
          value: contact.carPlatformNo__c,
          registrationNumber: contact.Registration_Number__c
        };
      });
  ...
  handleOutsideClick(event) {
    const isInsideModal = this.template
      .querySelector(".inputMultiple")
  ...
  handleInputChange(event) {
    ...
    this.inputs[index].chassisNumber =
      `${this.inputs[index].part1}-${this.inputs[index].part2}`;
    ...
    if (this.fuse && value !== "") {
      ...
      this.inputs = this.inputs.map((input, i) => ({
        ...
        emptylist:
          i === parseInt(index, 10) ? searchResults.length === 0 : false
      }));
    } else {
  ...
  handleSelection(event) {
    ...
    if (hyphenIndex !== -1) {
      ...
      console.log(
        "Updated inputs after selection:",
      ...
      this.inputs[index].chassisNumber = selectedValue;
      console.log(
        "Updated inputs after selection (without hyphen):",
        JSON.stringify(this.inputs)
      );
    }
    this.inputs[index].searchResults = this.inputs[index].searchResults.filter(
      (result) => result.value !== selectedValue
    );

    // Remove the selected value from optVehicles
    this.optVehicles = this.optVehicles.filter(
  ...
  handleSearchButtonClick() {
    if (this.inputs.length > 0) {
      ...
      if (filledInputs.length > 0) {
        ...
        this.showToast(
          "Error",
          "Please fill all chassis number fields.",
          "error"
        );
      }
    } else {
  ...
  handleSearch(filledInputs) {
    ...
    senddata({ chassisNumbers })
      .then((result) => {
        ...
        const vehicleIdsToAdd = this.vehicleInfo.map(
          (vehicle) => vehicle.carPlatformNo__c
        );
        this.filteredregisteredChassis = vehicleIdsToAdd;

        this.RegVehicle =
          this.registeredVehicles.length > 0 && this.vehicleInfo.length === 0;
        this.VehicleInfo =
          this.registeredVehicles.length === 0 && this.vehicleInfo.length > 0;
        this.Bothcase =
          this.registeredVehicles.length > 0 && this.vehicleInfo.length > 0;
